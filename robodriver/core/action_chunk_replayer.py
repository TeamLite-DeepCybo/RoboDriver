...
class ActionChunkReplayer:
    ...
    使用示例::
        ...
        replayer = ActionChunkReplayer(robot, fps=30)
        replayer.start()
    ...
    def load_chunk(self, action_chunk: np.ndarray, request_id: str = "") -> None:
        ...
        logger.info(
            f"[ActionChunkReplayer] chunk loaded: {arr.shape[0]} frames, "
    ...
    def start(self) -> None:
        ...
        self._thread = threading.Thread(
            ...
            name="ActionChunkReplayer",
            daemon=True,
        )
        self._thread.start()
        logger.info(f"[ActionChunkReplayer] started @ {self.fps} Hz")
    ...
    def stop(self) -> None:
        ...
        logger.info(
            f"[ActionChunkReplayer] stopped, total frames sent={self.frames_sent}"
        )
    ...
    def _replay_loop(self) -> None:
        """主回放循环：从当前 chunk 逐帧取 action 并下发。

        节拍控制与 replayer.py 的 replay() 函数完全一致：
        ...
        while self._running:
            ...
            try:
                ...
                logger.exception(
                    f"[ActionChunkReplayer] send_action failed at "
