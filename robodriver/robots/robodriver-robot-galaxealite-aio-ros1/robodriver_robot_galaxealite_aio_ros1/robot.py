...
from .config import GALAXEALITEAIORos1RobotConfig  # 改为ROS1配置类
from .status import GALAXEALITEAIORos1RobotStatus  # 改为ROS1状态类（需确保status.py已适配ROS1）
from .node import GALAXEALITEAIORos1RobotNode, ros_spin_thread  # 导入ROS1节点和spin函数


logger = logging_mp.getLogger(__name__)


class GALAXEALITEAIORos1Robot(Robot):  # 类名改为ROS1标识
    config_class = GALAXEALITEAIORos1RobotConfig  # 关联ROS1配置类
    name = "galaxealite-aio-ros1"  # 名称改为ROS1版本标识

    def __init__(self, config: GALAXEALITEAIORos1RobotConfig):
        ...
        self.status = GALAXEALITEAIORos1RobotStatus()
        if not rospy.core.is_initialized():
            rospy.init_node('ros1_recv_pub_driver', anonymous=True)
        else:
            logger.info(f"✅ 复用已存在的ROS节点：{rospy.get_name()}")
        
        self.robot_ros1_node = GALAXEALITEAIORos1RobotNode()  # 创建ROS1节点实例（替换ROS2节点）
    ...
    def connect(self):
        ...
        conditions = [
            ...
            (
                lambda: all(
                    any(name in key for key in self.robot_ros1_node.recv_leader)
                    for name in self.leader_motors
                ),
                lambda: [
                    name
                    for name in self.leader_motors
                    if not any(name in key for key in self.robot_ros1_node.recv_leader)
                ],
                "等待主臂关节角度超时",
            ),
            (
        ...
        if conditions[1][0]():
            leader_received = [
                name
                for name in self.leader_motors
                if any(name in key for key in self.robot_ros1_node.recv_leader)
            ]
    ...
    def get_action(self) -> dict[str, Any]:
        ...
        # 读取主臂数据（替换ROS2节点为ROS1节点）
        for comp_name, joints in self.leader_motors.items():
            for follower_name, follower in self.robot_ros1_node.recv_leader.items():
                if follower_name == comp_name:
                    for i, joint_name in enumerate(joints.keys()):
    ...
    def send_action(self, action: dict[str, Any]):
        ...
        if not self.is_connected:
            raise DeviceNotConnectedError(
                "GALAXEALITEAIORos1Robot is not connected. You need to run `robot.connect()`."
            )
    ...
    def update_status(self) -> str:
        ...
        # 更新主臂状态（替换ROS2节点为ROS1节点）
        for i in range(self.status.specifications.arm.number):
            match_name = self.status.specifications.arm.information[i].name
            for name in self.robot_ros1_node.recv_leader_status:
                if match_name in name:
                    self.status.specifications.arm.information[i].is_connect = (
                        True if self.robot_ros1_node.recv_leader_status[name] > 0 else False
                    )
    ...
    def disconnect(self):
        if not self.is_connected:
            raise DeviceNotConnectedError(
                "GALAXEALITEAIORos1Robot is not connected. You need to run `robot.connect()` before disconnecting."
            )
