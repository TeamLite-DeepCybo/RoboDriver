...
from .config import DobotNova2Ros2RobotConfig
from .status import DobotNova2Ros2RobotStatus
...
class DobotNova2Ros2Robot(Robot):
    config_class = DobotNova2Ros2RobotConfig
    name = "galaxealite-aio-ros2"

    def __init__(self, config: DobotNova2Ros2RobotConfig):
        ...
        self.status = DobotNova2Ros2RobotStatus()
        if not rclpy.ok():
            rclpy.init()
        self.robot_ros2_node = DobotNova2Ros2RobotNode()  # 创建节点实例
