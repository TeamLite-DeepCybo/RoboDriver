# robodriver_robot_unitree_g1_zmq/node.py
...
class UnitreeG1Node:
    ...
    def __init__(
        self,
        zmq_endpoint: str = "tcp://192.168.1.156:5555",
        ...
        logger.info(f"[ZMQ Node] UnitreeG1Node connected to {zmq_endpoint}")
                        ...
                        if message_parts:
                            ...
                            if b' ' in message:
                                ...
                                try:
                                    ...
                                    self.recv_leader["leader_joints"] = processed_data
                                    self.recv_leader_status["leader_joints"] = CONNECT_TIMEOUT_FRAME
                    
                                    self.recv_follower["follower_joint"] = processed_data
    ...
    def extract_lowstate_data(self, raw_data: Dict) -> Dict[str, Any]:
        """从原始LowState数据中提取关键信息"""
        extracted = {
            "timestamp": raw_data.get("_timestamp", ""),
            "frame_count": raw_data.get("_frame_count", 0),
            "mode": raw_data.get("mode_machine", 0),
        ...
        # 提取IMU数据
        if "imu_state" in raw_data:
            imu = raw_data["imu_state"]
            extracted["imu"] = {
        ...
        # 提取电机状态
        if "motor_state" in raw_data and isinstance(raw_data["motor_state"], list):
            for i, motor in enumerate(raw_data["motor_state"]):
                motor_info = {
                    "index": i,
                    "mode": motor.get("mode", 0),
                    "q": float(motor.get("q", 0.0)),
                    "dq": float(motor.get("dq", 0.0)),
                    "ddq": float(motor.get("ddq", 0.0)),
                    "tau_est": float(motor.get("tau_est", 0.0)),
                    "temperature": motor.get("temperature", [0, 0]),
                    "vol": float(motor.get("vol", 0.0)),
                    "sensor": motor.get("sensor", [0, 0]),
                    "motorstate": float(motor.get("motorstate", 0.0)),
               }
                extracted["motor_state"].append(motor_info)
        
        # 提取无线遥控数据
        if "wireless_remote" in raw_data and isinstance(raw_data["wireless_remote"], list):
            extracted["wireless_remote"] = raw_data["wireless_remote"]
