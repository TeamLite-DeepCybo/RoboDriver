# robodriver_robot_unitree_g1_sdk_Py/robot.py
...
class UnitreeG1Robot(Robot):
    config_class = UnitreeG1RobotConfig
    name = "unitree_g1_sdk_py"

    def __init__(self, config: UnitreeG1RobotConfig):
        ...
        self.robot_ros2_node = UnitreeG1Node()
    ...
    def get_observation(self) -> dict[str, Any]:
        ...
        for comp_name, joints in self.follower_motors.items():
            ...
            vec = state_dict.get("motor_state")
            if vec is None:
                continue           
            ...
            for idx, joint in enumerate(joint_names):
                if idx >= len(vec):
                    break
                obs_dict[f"follower_{joint}.pos"] = float(vec[idx].get("q"))
    ...
    def get_action(self) -> dict[str, Any]:
        ...
        for comp_name, joints in self.leader_motors.items():
            ...
            for idx, joint in enumerate(joint_names):
                if idx >= len(vec):
                    break
                act_dict[f"leader_{joint}.pos"] = float(vec[idx].get("q"))
