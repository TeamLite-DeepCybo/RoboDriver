...
class SimFrankaAIODoraRobotNode(DoraRobotNode):
    def __init__(self):
        self.node = Node("robodriver-sim")
    ...
    def _handle_input_event(self, event):
        ...
        with self.lock:
            ...
            if 'image' in event_id:
                ...
                self._process_joint(event_id, data, meta_data)
    ...
    def _process_joint(self, event_id, data, metadata):
        """处理关节数据"""
        if data is not None:
            ...
            if "follower" in event_id:
                # scalar_value = float(data.item())
                for idx, name in enumerate(metadata["names"]):
                    self.recv_joint_follower[name] = data[idx]
